...
function dailyData(data) {
    ...
    for (let i = 0; i <= 6; i++) {
        ...
        let {humidity} = data.daily[dayNum]
        ...
        `<div class="daily-main-data">
            ...
            <button class="arrow-btn btn-hide" onclick="secunData(${dayNum})"><i id="rotateArrow${dayNum}" class="fas fa-chevron-down"></i></button>
        </div>
        <div id="secunData${dayNum}" class="daily-secun-data">
...
function secunData(dailyDataSubmenuNum) { 
    const contDailyDataSubmenu = document.getElementById(`secunData${dailyDataSubmenuNum}`);
    const arrowOpenSubmenu = document.getElementById(`rotateArrow${dailyDataSubmenuNum}`);
    contDailyDataSubmenu.classList.toggle("change-height")
    arrowOpenSubmenu.classList.toggle("rotate-arrow")
